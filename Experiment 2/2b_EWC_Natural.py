'''
Experiment 2b - Neural Network equipped with EWC in a Natural language corpus
...
#train EWC in sense1-sense2 order
def train12(t1, t2, wti, itw, vocab_len, runs, embed_size, lr, negsamps, fishruns,lam,  target, sense1, sense2):
    ...
    for i in range(0, num_runs):
        ...
        fisher = compute_fisher(train_inputs, train_labels, target_ind, sess, fishruns, varlist , hidden_out)
        ...
        loss2 = set_ewcloss(varlist,saved_vars, lam, fisher, loss)
...
#train EWC in sense2-sense1 order  
def train21(t1, t2, wti, itw, vocab_len, runs, embed_size, lr, negsamps, fishruns,lam,  target, sense1, sense2):
    # number of runs
    ...
    for i in range(0, num_runs):
        ...
        # update weights for task 2
        for e in range(0, len(input_feed_task2)): #for all items in input
            x = sess.run([loss2, optimizer], feed_dict = {train_inputs: [input_feed_task2[e]], train_labels: [output_feed_task2[e]]})
...
def run(f1,f2, runs, embed_size, lr, negsamps, fishruns, lam, target, sense1, sense2):
    '''f1 = filepath 1, f2 = filepath2, negsamps = number of negative samples for nce loss to use,
    fishruns = number of samples to train fisher, lam = lambda parameter for EWC, target = homonym'''
    ...
    topic2_sent = seperate_sentences(fetch_clean_data(file2)[0])## collect topic 1 sentences
    ...
    #train ewc on sense1-sense2 order
    p2 = Process(target= train12, args=(topic1_sent, topic2_sent, word_to_index, index_to_word,len(vocab), 
                                        runs, embed_size, lr,negsamps, fishruns, lam, target,sense1,sense2))
    p2.start()
    
    #train ewc on sense2-sense1 order
...
if __name__ == '__main__':
    ...
    run(f1 =file1p, f2 = file2p, runs = 100, embed_size = 50,lr= 0.3, negsamps= 64, fishruns=200, 
        lam = 100, target='firm', sense1='hire', sense2='connective')
